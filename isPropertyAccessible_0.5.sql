...
--!!! TODO: clean temporary tables, unused any more fields/columns, test again for levels, check/implement root C0
...
CREATE OR REPLACE FUNCTION isPropertyAccessible(property_in integer, company_in integer) RETURNS integer AS $$
	DECLARE passed_routes INTEGER;
		...
		-- filtering graph_history for *property_in* based on Xclusivity rules

		DELETE FROM graph_history where property=$1 ;

		DROP TABLE IF EXISTS temp_graph_history;   

		CREATE TABLE IF NOT EXISTS temp_graph_history (
		...
		DELETE FROM graph_history where property=$1 ;
		INSERT INTO graph_history SELECT row_number() OVER()  AS nextval, source, target, strength, property FROM temp_graph_cleaning_excl order by property, nextval; 
		...
		WITH RECURSIVE graph AS (
		    ...
		    SELECT o.source
			  ...
			  ,g.depth + 1
		    FROM   graph g
		    JOIN   temp_graph_cleaning_excl o ON o.source = g.target
